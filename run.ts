...
async function init(): Promise<void> {
  ...
  switch (TOKEN_SYMB) {
    case "WSOL": {
      ...
      quoteMaxPoolSizeAmount = new TokenAmount(
        quoteToken,
        MAX_POOL_SIZE,
        false
      );
  ...
  logger.info(
    `Pool size max <: ${
      quoteMaxPoolSizeAmount.isZero()
        ? "false"
        : quoteMaxPoolSizeAmount.toFixed()
    } ${quoteToken.symbol}`
  );
  ...
  if (!tokenAccount) {
    logger.error(
      `---> Put SOL in your wallet and swap SOL to WSOL at https://jup.ag/ <---`
    );
    throw new Error(
...
export async function processRaydiumPool(
  ...
  if (!quoteMinPoolSizeAmount.isZero()) {
    ...
    if (poolSize.lt(quoteMinPoolSizeAmount) || rugRiskDanger) {
      logger.warn(
        `------------------- POOL SKIPPED | (${poolSize.toFixed()} ${
          quoteToken.symbol
        }) ------------------- `
      );
    } else {
      logger.info(
        `--------------!!!!! POOL SNIPED | (${poolSize.toFixed()} ${
          quoteToken.symbol
        }) !!!!!-------------- `
      );
    }
    ...
    logger.info(
      `Pool Open Time: ${new Date(
        parseInt(poolState.poolOpenTime.toString()) * 1000
      ).toLocaleString()}`
    );
    logger.info(`--------------------- `);
  ...
  if (!quoteMaxPoolSizeAmount.isZero()) {
    ...
    if (poolSize.gt(quoteMaxPoolSizeAmount)) {
      logger.warn(
        ...
        `Skipping pool, > ${quoteMaxPoolSizeAmount.toFixed()} ${
          quoteToken.symbol
        }`,
  ...
  do {
    try {
      ...
      logger.info(
        ...
        `Confirmed sell tx... Sold at: ${value}\tNet Profit: ${
          netChange * 100
        }%`
      );
      return true;
